...
public final class Processor {
    ...
    public int readRegister(int number) {
        assert (number >= 0 && number < numUserRegisters);
    ...
    public TranslationEntry readTLBEntry(int number) {
        assert (usingTLB);
        assert (number >= 0 && number < tlbSize);
    ...
    private class MipsException extends Exception {
        public MipsException(int cause) {
            assert (cause >= 0 && cause < exceptionNames.length);
    ...
    private class Instruction {
        ...
        private void print() {
            ...
            assert (spaceIndex != -1 && spaceIndex == name.lastIndexOf(' '));
            ...
            for (int i = 0; i < args.length; i++) {
                ...
                if (i + 1 < args.length) {
                    ...
                    assert (maxCharsPrinted - minCharsPrinted < 8);
